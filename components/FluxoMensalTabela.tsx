...
function CelulaDia({ valor, ehForecast, className }: { valor: number | null; ehForecast?: boolean; className?: string }) {
  return (
...
export default function FluxoMensalTabela({ dados, onAbrirDrilldown }: Props) {
  const [expandidoFixo, setExpandidoFixo] = useState(false)
  const [expandidoVariavel, setExpandidoVariavel] = useState(false)
  const hoje = hojeISO()

  const orcadoPorConta = new Map(dados.orcadoXRealizadoFixo.map((o) => [o.id, o]))
  const orcadoPorFornecedor = new Map(dados.orcadoXRealizadoVariavel.map((o) => [o.id, o]))
      ...
      <table className="text-xs min-w-max">
        <thead>
          <tr className="border-b border-gray-200">
            ...
            {dados.dias.map((d) => {
              ...
              return (
                <th key={d} className={`px-2 py-2 text-right font-medium min-w-[52px] ${d === hoje ? 'bg-pink-50 text-pink-700' : 'text-gray-500'}`}>
        ...
        <tbody className="divide-y divide-gray-100">
            ...
            <>
              <tr className="bg-blue-50/50">
                ...
                {dados.faturamentoPorDia.map((v, i) => (
                  <CelulaDia key={i} valor={v} ehForecast={dados.faturamentoEhForecastPorDia[i]} />
                ))}
              ...
              <tr>
                ...
                {dados.metaDiariaPorDia.map((v, i) => <CelulaDia key={i} valor={v} />)}
              ...
              <tr>
                ...
                {dados.deltaPorDia.map((v, i) => (
                  <td key={i} className={`px-2 py-1.5 text-right whitespace-nowrap ${v == null ? 'text-gray-400' : v >= 0 ? 'text-green-600' : 'text-amber-600'}`}>
                    {formatCompacto(v)}
                  </td>
                ))}
              ...
              <tr className="border-b-2 border-gray-200">
                ...
                {dados.gapAcumuladoPorDia.map((v, i) => (
                  <td key={i} className={`px-2 py-1.5 text-right whitespace-nowrap font-medium ${v == null ? 'text-gray-400' : v >= 0 ? 'text-green-600' : 'text-amber-600'}`}>
                    {formatCompacto(v)}
                  </td>
                ))}
                <td className="px-3 py-1.5" />
          ...
          <tr className="bg-green-50/50">
            ...
            {dados.entradasCaixaPorDia.map((v, i) => <CelulaDia key={i} valor={v} ehForecast={dados.entradasCaixaEhForecastPorDia[i]} />)}
          ...
          {dados.entradasCaixaPorCategoria.map((c) => (
            <tr key={c.categoria}>
              ...
              <td colSpan={dados.dias.length} className="px-2 py-1 text-right text-gray-400 italic">só mês inteiro</td>
          ...
          <tr className="border-t-2 border-gray-200">
            <td className="sticky left-0 bg-white px-3 py-1.5">
              <button onClick={() => setExpandidoFixo((v) => !v)} className="flex items-center gap-1 font-semibold text-gray-700 hover:text-pink-700">
                {expandidoFixo ? <ChevronDown size={14} /> : <ChevronRight size={14} />} Saídas Custo Fixo
              </button>
            </td>
            {dados.saidasFixoPorDia.map((v, i) => <CelulaDia key={i} valor={-v} className={v > 0 ? 'text-red-600' : ''} />)}
            <td className="px-3 py-1.5 text-right font-semibold text-red-600">{formatBRL(dados.totalSaidasFixo)}</td>
          </tr>
          {expandidoFixo &&
            dados.saidasFixoPorConta.map((linha) => {
              const orcado = orcadoPorConta.get(linha.id)
              return (
                <tr
                  key={linha.id}
                ...
                >
                  ...
                  {linha.porDia.map((v, i) => <CelulaDia key={i} valor={v > 0 ? -v : 0} />)}
          ...
          <tr className="border-t-2 border-gray-200">
            <td className="sticky left-0 bg-white px-3 py-1.5">
              <button onClick={() => setExpandidoVariavel((v) => !v)} className="flex items-center gap-1 font-semibold text-gray-700 hover:text-pink-700">
                {expandidoVariavel ? <ChevronDown size={14} /> : <ChevronRight size={14} />} Saídas Custo Variável
              </button>
            </td>
            {dados.saidasVariavelPorDia.map((v, i) => <CelulaDia key={i} valor={-v} className={v > 0 ? 'text-red-600' : ''} />)}
            <td className="px-3 py-1.5 text-right font-semibold text-red-600">{formatBRL(dados.totalSaidasVariavel)}</td>
          </tr>
          {expandidoVariavel &&
            dados.saidasVariavelPorFornecedor.map((linha) => {
              const orcado = orcadoPorFornecedor.get(linha.id)
              return (
                <tr
                  key={linha.id}
                  className="hover:bg-gray-50 cursor-pointer"
          ...
          <tr className="border-t-2 border-gray-300 bg-gray-50">
            ...
            {dados.saldoDiaPorDia.map((v, i) => (
              <td key={i} className={`px-2 py-1.5 text-right whitespace-nowrap font-medium ${v >= 0 ? 'text-green-600' : 'text-red-600'}`}>{formatCompacto(v)}</td>
            ))}
            <td className="px-3 py-1.5" />
          </tr>
